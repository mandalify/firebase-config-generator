...
export type CloudFunctionRuntimes = "nodejs10" | "nodejs12" | "nodejs14";
...
export type DatabaseSingle = {
  rules: string;
} & Deployable;

export type DatabaseMultiple = ({
  rules: string;
} & RequireAtLeastOne<{
  instance: string;
  target: string;
...
export type HostingSource = { source: string } | { regex: string };
...
export type StorageSingle = {
  rules: string;
  target?: string;
} & Deployable;

export type StorageMultiple = ({
  rules: string;
  bucket: string;
  target?: string;
} & Deployable)[];
